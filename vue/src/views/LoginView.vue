<template>
  <div class="w-full h-full flex flex-col justify-center gap-16 items-center bg-[var(--surface-ground)]">
    ...
    <card class="w-80">
      <template #title><h1 class="text-center">{{ isNewUser ? 'Create Account' : 'Login Using' }} </h1></template>
      <template #content>
        ...
        <div v-else class="flex gap-8 justify-center flex-col">
          <file-upload accept="image/*" v-model="profilePicture" :file-limit="1" @select="(e) => profilePicture = e.files[0]">
            ...
            <template #content="{ files, removeFileCallback }">
              ...
              <i :class="`pi pi-${files.length ? 'times' : 'upload'} absolute right-0 bottom-0 bg-[var(--primary-color)] p-2 rounded-full text-xs`" @click="files.length ? removeFileCallback(0) : chooseFile()"></i>
            </template>
          </file-upload>
          <span>
            <span class="p-float-label">
              <input-text id="username" :class="{ 'p-invalid': username.error, 'w-full': true }" v-model="username.text" @update:model-value="() => username.error = ''"/>
...
const authWithOAuth = (provider: string) => {
    pb.collection('users').authWithOAuth2({ provider: provider, scopes: ['error'] })
    .then(res => {
      if (res.meta?.isNew) {
